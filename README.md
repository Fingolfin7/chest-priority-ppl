...
A simple, phone-first, chest-prioritized rolling Push/Pull/Legs workout reference. The sequence continues across five weekdays instead of resetting every Monday, so missed sessions never require rescheduling the week.
...
- Device-local work-set history with previous-session context and double-progression suggestions
...
Build the production site with `npm run build`. Output is written to `dist/`.
